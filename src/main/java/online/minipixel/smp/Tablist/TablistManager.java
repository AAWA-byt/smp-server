package online.minipixel.smp.Tablist;
...
public class TablistManager {
    ...
    public void setPlayerTeam(Player player) {
        ...
        for (Player target : Bukkit.getOnlinePlayers()) {
            if (target.hasPermission("minipixel.admin")) {
                padmin.addEntry(target.getName());
                continue;

            } else if (target.hasPermission("minipixel.mod")) {
                pmod.addEntry(target.getName());
                continue;

            } else if (target.hasPermission("minipixel.dev")) {
                pdev.addEntry(target.getName());
                continue;

            } else if (target.hasPermission("minipixel.sup")) {
                psup.addEntry(target.getName());
                continue;

            } else if (target.hasPermission("minipixel.builder")) {
                pbuilder.addEntry(target.getName());
                continue;

            } else if (target.hasPermission("minipixel.vip+")) {
